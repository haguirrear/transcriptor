...
async def generate_transcription(audio_file: UploadFile):
    ...
    if len(file_bytes) > max_size_bytes:
        ...
        for start_ms in range(0, total_duration, 10 * 60 * 1000):  # Segmentos de hasta 10 minutos
            # Calcula el final del segmento actual
            end_ms = start_ms + 10 * 60 * 1000
            # Asegúrate de no sobrepasar la duración total del audio
            segment = original_audio[start_ms:end_ms] if end_ms < total_duration else original_audio[start_ms:]
